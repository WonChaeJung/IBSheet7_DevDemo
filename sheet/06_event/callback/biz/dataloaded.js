...
function LoadPage() {
	...
	var initSheet = {
		...
		Cols : [
			{Header:"순번",Type:"Seq",Width:35, SaveName:"param0", Align:"Center"   },
...
function mySheet_OnRowSearchEnd(row) {
	...
	if(!_ibsUtil[fnName]){
		...
		if(mySheet.GetCellValue(row, 5) > 10000000){
			mySheet.SetCellFontColor(row ,5 ,"#FF0000");
			mySheet.SetCellFontBold(row ,5, 1);
		}
	}
}

function mySheet_OnSearchEnd(a, b) {
	var fnName = arguments.callee.name.split("_");
	...
	if(!_ibsUtil[fnName]){
		addLog("이벤트명 : " + fnName + "          인자 값 : a : " + a+" b : "+ b);
	}
}
...
function doAction(sAction) {
	switch(sAction) {
		case "search" : {
			mySheet.DoSearch("./data/searchend_data.js");
			break;
		}
	}
}
